...
internal sealed class DialogueExtensions
{
	...
	private static void AStatus_Begin_Postfix(AStatus __instance, State s, Combat c, ref int __state)
	{
		...
		if (__instance.status == ModEntry.Instance.Lars_Character.MissingStatus.Status && __state > 0 && s.ship.Get(ModEntry.Instance.Lars_Character.MissingStatus.Status) <= 0)
	...
	{
		var currentCardsInDeck = g.state.deck.Select(card => card.uuid).ToHashSet();
